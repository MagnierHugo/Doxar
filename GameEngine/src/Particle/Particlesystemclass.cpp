#include "Particle/particlesystemclass.hpp"
...
bool ParticleSystemClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* textureFilename) {
	...
	//Load the texturethat is used the particules
	...
	//initialize the particile system
	result = InitializeParticleSystem();
	if (!result) {
		return false;
	}

	//Create the buffers that will be use to render the particle with
...
void ParticleSystemClass::Shutdown() {
	...
	//release the texture used for particles
	ReleaseTexture();
...
bool ParticleSystemClass::Frame(float frameTime, ID3D11DeviceContext* deviceContext) {
	...
	//Update the dynamic vertex buffer with the new position of each particles 
...
ID3D11ShaderResourceView* ParticleSystemClass::GetTexture() {
	ShaderRessourceViewVariant variant = m_Texture->GetTexture();
	auto texture = std::get_if<ID3D11ShaderResourceView*>(&variant);
	return *texture;
}
